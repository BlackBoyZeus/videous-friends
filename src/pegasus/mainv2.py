...
# ======================================== GUMROAD GATEWAY ========================================
def tk_write(tk_string1): #Makes a popup displaying text string tk_string1 
    window = tkinter.Tk()
    window.title("Pegasus Video Editor")
...
def mainApp():
  ...
  app.title("Pegasus Video Editor")

  headerlabel = customtkinter.CTkLabel(master=app, text="Pegasus Video Editor", text_font=("Arial", 32))
  headerlabel.place(relx=0.5, rely=0.1, anchor=tkinter.CENTER)

  #Sliders
  slider_var = tkinter.IntVar(value=1)
  def slider_event(valueSlider):
    sliderlabel2 = customtkinter.CTkLabel(master=app, text= str(int(valueSlider)))
    sliderlabel2.place(relx=0.8, rely=0.3, anchor=tkinter.CENTER)

    print(valueSlider)

  sliderlabel1 = customtkinter.CTkLabel(master=app, text="Increase to speed up the program\n(may affect results)")
  sliderlabel1.place(relx=0.2, rely=0.3, anchor=tkinter.CENTER)
  ...
  def paddingSlider_event(valueSlider):
    ...
    paddingSliderlabel2.place(relx=0.8, rely=0.4, anchor=tkinter.CENTER)

    print(valueSlider)

  paddingSliderlabel1 = customtkinter.CTkLabel(master=app, text="Number of seconds to pad each frame")
  paddingSliderlabel1.place(relx=0.2, rely=0.4, anchor=tkinter.CENTER)
  ...
  paddingSlider.place(relx=0.5, rely=0.4, anchor=tkinter.CENTER)

  #Dropdown menu
  comboboxlabel1 = customtkinter.CTkLabel(master=app, text="What Activity Should I Look For?")
  comboboxlabel1.place(relx=0.5, rely=0.5, anchor=tkinter.CENTER)
  ...
  combobox.place(relx=0.5, rely=0.6, anchor=tkinter.CENTER)

  #CTkButtons

  runninglabel = customtkinter.CTkLabel(master=app, text="It might take a few minutes depending on the length of your video, please hang on :)", text_font=("Arial", 12))
  runninglabel.place(relx=0.5, rely=0.7, anchor=tkinter.CENTER)
  ...
  def button_function():
    ...
    successMessage = f"Video successfully saved as {outputVideoName}"
    #except:
    ...
    successlabel = customtkinter.CTkLabel(master=app, text=successMessage, text_font=("Arial", 12))
  ...
  button = customtkinter.CTkButton(master=app, text="Select Your Video and Run the Program!", fg_color="purple", hover_color="white", command=button_function)
  button.place(relx=0.2, rely=0.8, anchor=tkinter.CENTER)

  endButton = customtkinter.CTkButton(master=app, text="Close Program", fg_color="red", hover_color="white", command=app.destroy)
  endButton.place(relx=0.8, rely=0.8, anchor=tkinter.CENTER)
